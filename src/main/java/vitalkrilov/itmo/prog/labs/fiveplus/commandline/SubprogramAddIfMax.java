...
class SubprogramAddIfMax extends SubprogramTemplate {
    SubprogramAddIfMax(CommandLineWorker commandLineWorker) {
    ...
    public Result<Object, String> execute(List<String> args) {
        ...
        {
            var res = this.commandLineWorker.collectionManager.requestAddIfMax(person);
            if (res.isErr()) return new Result.Err<>(res.getErr());
