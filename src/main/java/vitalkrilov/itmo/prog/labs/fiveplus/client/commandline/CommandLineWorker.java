...
import vitalkrilov.itmo.prog.labs.fiveplus.client.collection.CollectionManager;
...
public class CommandLineWorker {
    ...
    static {
        ...
        c = new Command("filter_starts_with_name", "Вывести элементы, значение поля name которых начинается с заданной подстроки.",
                new Command.Argument(Command.ArgumentType.POSITIONAL, "nameStart", "подстрока"));
        commandsDatabase.put(c.name, c);
    ...
    public int run(List<String> args) {
            ...
            switch (args.get(i)) {
                case "-h", "--help": {
                    System.out.print("Options:\n" +
                            "  -h, --help                 Print help\n" +
                            "  -H, --hostname <HOSTNAME>  Choose hostname to connect to [default: localhost]\n" +
                            "  -p, --port <PORT>          Choose port of target host [default: 7777]\n");
                    return 0;
                }
                case "-H", "--hostname", "-p", "--port": {
                    options.add(args.get(i));
                    break;
                }
                default: {
                    ...
                    switch (option) {
                        case "-H", "--hostname": {
                            hostname = args.get(i);
                            if (hostname.isEmpty()) {
                                Logger.logError("An empty hostname is not allowed.");
                                return 1;
                            }
                            break;
                        }
        ...
        this.collectionManager = new CollectionManager(hostname, port);
        Logger.logInfo("Testing connection to %s:%d...", hostname, port);
        var conRes = collectionManager.testConnection();
        if (conRes.isErr()) {
            Logger.logError("Unable to connect to server: " + conRes.getErr());
            return 1;
        }
        Logger.logInfo("Successfully connected. Starting shell...");
